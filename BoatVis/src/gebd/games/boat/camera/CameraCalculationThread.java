...
public class CameraCalculationThread implements DestoryableThread {
    ...
    public Vector3f getPixelAt(int x, int y) {
        int byteStartOffset = ((y * resolutionWidth) + x) * BYTES_PER_PIXEL;
        currentRead.position(byteStartOffset);

        //RGBA.
        float red = convertByteToFloat0to1(currentRead.get());
        float green = convertByteToFloat0to1(currentRead.get());
        float blue = convertByteToFloat0to1(currentRead.get());
        //float alpha = convertByteToFloat0to1(currentRead.get());

        return new Vector3f(red, green, blue);
    }

    private float convertByteToFloat0to1(byte givenByte) {
