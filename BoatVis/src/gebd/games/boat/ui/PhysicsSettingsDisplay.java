...
public class PhysicsSettingsDisplay extends UserInterface implements WaveSettings {
    ...
    protected SettingsHolderWithParents<PhysicsSettings> physicsSettings;

    protected SimpleQuad backgroundQuad;

    protected DoubleTextArea physicsSpeedModifier;

    protected DoubleTextArea leftMotorForceTextArea;
    protected DoubleTextArea rightMotorForceTextArea;

    protected TextButton useSimpleWater;
    protected DoubleTextArea waveHeight;
    protected DoubleTextArea waveDirection;
    protected DoubleTextArea waveFrequency;
    protected DoubleTextArea distanceBetweenWaves;

    protected String2D usePhysicsBoatText;
    protected TextButton usePhysicsBoatButton;

    protected TextButton setPhysicsBoatPositionButton;
    protected boolean setPhysicsBoatPosition = false;

    protected TextButton setPhysicsBoatRotationButton;
    protected boolean setPhysicsBoatRotation = false;

    protected Button reduceButton;

    protected TextButton resetBoatPositionButton;

    protected Vector3f defaultBoatPosition = new Vector3f();
    protected Quat4f defaultBoatRotation = new Quat4f();

    protected TextButton lidar5HzButton;
    protected TextButton lidar10HzButton;
    protected TextButton lidar20HzButton;
    protected HashMap<LidarHelper.LidarRotationSpeed, TextButton> lidarSpeedMap = new HashMap<>();

    private HashMap<PhysicsSettings, DoubleTextArea> settingsMap = new HashMap<>();

    private InputLatch positionSetter = KeyboardInputLatchGenerator.generateInputLatchForKey(Keyboard.KEY_P);
    private InputLatch rotationSetter = KeyboardInputLatchGenerator.generateInputLatchForKey(Keyboard.KEY_R);
    ...
    public PhysicsSettingsDisplay(Vector2f relativePosition, Vector2f size, float rotation, final BoatVis boatVis,
        ...
        leftMotorForceTextArea = addComponentToTop(new DoubleTextArea(new Vector2f(-75, 100), new Vector2f(100, 20), "Courier", Font.PLAIN, -200.0, 200.0){
            ...
            public void onDeselect() {
                super.onDeselect();
                boatVis.getPhysicsBoat().applyConstantMotorForceOnLeftPontoon((float) getCurrentValue());
            }
        });
        ...
        rightMotorForceTextArea = addComponentToTop(new DoubleTextArea(new Vector2f(75, 100), new Vector2f(100, 20), "Courier", Font.PLAIN, -200.0, 200.0){
            ...
            public void onDeselect() {
                super.onDeselect();
                boatVis.getPhysicsBoat().applyConstantMotorForceOnRightPontoon((float) getCurrentValue());
            }
        });
